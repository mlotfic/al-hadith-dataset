...
def get_hadith_info(soup, islmwy_page):
    """
    ...
    container = soup.find('div', class_='bookcontent-dic', id='pagebody')
    
    # Extract html_hadith
    hadith_info['html_hadith_narrators'] = container
    ...
    if container:
        ...
        if hashiya_span:
            hadith_info['hashiya_title'] = hashiya_span.next_sibling.strip()
        ...
        hadith_info['text_hadith_narrators'] = re.sub(r'\s+', ' ', text_hadith).strip()
        ...
        if first_quran_link:
            ...
            if subject:
                hadith_info['hadith_subject'].append({
        ...
        if hadith_span:
            ...
            # Remove extra spaces and tabs
            hadith_info['hadith_content'] = re.sub(r'\s+', ' ', hadith_text).strip()
            hadith_info['hadith_content_url'] = hadith_span.find('span', class_='hadithatt').text.strip() if hadith_span else 'N/A'
        ...
        for span in verse_spans:
            ...
            if matches:
                for href, verse in matches:
                    hadith_info['quran_verses'].append({
                        'verse'     : verse.strip(),
        ...
        if match:
            ...
            hadith_info['narrators_chains'] = narrators_chains
        else:
